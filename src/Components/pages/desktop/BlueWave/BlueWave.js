                    ...
                    <section className="DeppartmentFile">
                        <div className="overFlow">
                            <div className="chooseBuilding" style={{backgroundImage: "url(" + Background + ")" }}>
                                <TextToHtml format='h2' NameClass="textParagraph" text={page.chooseBuilding_h2} />
                                { buildings.map( building =>
                                <button key ={ building.id } className= {
                                    this.state.building === building.id ? 'building' + building.id + ' active' :  'building' + building.id 
                                    } onClick={() => this.selectTochniotByPicture(building)}>
                                    <BuildingButton btnNumber={building.id} buildingTitle={building.title} currentNumber={this.state.buildingData.id} />
                                </button>
                                )}
                            </div>
                        </div>
